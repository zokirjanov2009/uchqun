...
const ParentManagement = () => {
  ...
  const handleDelete = async (parentId) => {
    ...
    try {
      dataStore.deleteParent(parentId);
      success('Parent account deleted successfully');
      loadParents();
    } catch (error) {
      console.error('Error deleting parent:', error);
      showError('Error deleting parent account');
    }
  };
    ...
    try {
      if (editingParent) {
        ...
        if (!updateData.password) {
          ...
          const existing = dataStore.getParents().find(p => p.id === editingParent.id);
          if (existing) {
            updateData.password = existing.password;
          }
        }
        dataStore.updateParent(editingParent.id, updateData);
        success('Parent account updated successfully');
      } else {
        // Create new parent
        dataStore.createParent(formData);
      ...
      showError(error.message || 'Error saving parent account');
    }
  };
